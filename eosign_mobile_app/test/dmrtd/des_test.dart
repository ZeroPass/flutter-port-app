...
void main() {
  final zeroIV = Uint8List(TDESCipher.blockSize);

  test('TDES Encryption/Decryption with no padding', () {
    ...
    final Eifd   = TDESCipher(key: tvKenc, iv: zeroIV).encrypt(tvS, padData: false);
    expect( Eifd, tvEifd );

    final dS = TDESCipher(key: tvKenc, iv: zeroIV).decrypt(Eifd, paddedData: false);
    expect(dS, tvS);  
    ...
    final Eic   = TDESEncrypt(key: tvKenc, iv: zeroIV, data: tvR, padData: false);
    expect( Eic, tvEic );

    final dR = TDESDecrypt(key: tvKenc, iv: zeroIV, edata: Eic, paddedData: false);
    expect( dR, tvR );  
  });

  test('TDES Encryption/Decryption with padding', () {
    ...
    var edata = TDESCipher(key: tvKSenc, iv: zeroIV).encrypt(tvData, padData: true);
    expect( edata, tvEData );

    var data = TDESCipher(key: tvKSenc, iv: zeroIV).decrypt(edata, paddedData: true);
    expect( data, tvData ); 
    ...
    edata = TDESCipher(key: tvKSenc, iv: zeroIV).encrypt(tvData); // should be padded by default
    expect( edata, tvEData );

    data = TDESCipher(key: tvKSenc, iv: zeroIV).decrypt(edata);   // should be unpadded by default
    expect( data, tvData ); 
    ...
    edata   = TDESEncrypt(key: tvKSenc, iv: zeroIV, data: tvData); // should be padded by default
    expect( edata, tvEData );

    data = TDESDecrypt(key: tvKSenc, iv: zeroIV, edata: edata);   // should be unpadded by default
    expect( data, tvData ); 
  });
}